...
npx eas-cli build:configure                 # generates eas.json, adds extra.eas.projectId to app.json
...
frontend/
  ...
  eas.json     → new (T3) — build profiles; development profile's ios config set to
                 `"simulator": true`
...
- **This module is manual-verification-heavy by nature**, same as Android's T5/T6:
  ...
  1. **Simulator, via EAS** (Decision 6): `expo prebuild --platform ios --clean`
     ...
     directly (T2, done). Then `eas build --profile development --platform ios`
